<?php
    if (!empty($_POST)) {   // We have post vars!
        $_POST['routing-chain'] = array_reverse($_POST['routing-chain']);   // Reverse the array so we can get the right chop order.
        ...
        <div class="row gap">
            <div class="col" style="flex: 1 0 auto">
                ...
                <div class="row gap" style="justify-content: space-between; flex-wrap: nowrap">
                    ...
                    <div class="col" style="width: 50%">
                        <label for="available-groups">Available Routing Groups [<a href="?view=admin&page=routing-groups">Manage Routing Groups</a>]</label>
...
<script>
    $(document).ready(function(){
                ...
                $.post("<?php echo CORRESPONDENCE_TYPE_NEW?>", {
                    ...
                    if (json['status'] == 200) {
                        ...
                        $.each(json['message'], function (k, v) { 
                            if (v.name == typeName) {
                                // If this is the group we just made, select it in the list and clear the routing chain.
                                // Also 
                                $("#correspondence-type").append("<option value=\"" + v.id + "\" selected>" + v.name + "</option>");
                                $("#routing-chain").empty();
                                getLists();
                            } else {
        ...
        $("#remove-type-btn").click(function() {
            ...
            if (typeID !== null) {
                if (confirm("WARNING: Deleting '" + $("#correspondence-type option:selected").text() + "' will also delete all correspondence of this type being routed. Continue?")) {
        ...
        $("#routing-chain-form").on("submit", function(e) {
            if (confirmUpdate == false) {
                // Need to advise user of the repercussions.
                e.preventDefault();
                if (confirm("WARNING: This will affect all correspondence of this type being routed. Continue?")) {
                    confirmUpdate = true;
                    $("#routing-chain option").prop("selected", true);
                    $("#routing-chain-form").submit();
                }
            }
        });
